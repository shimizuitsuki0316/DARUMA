...
class CNN3_128_9_NN2_121_128(nn.Module):
    ...
    def forward(self, x):
        x = F.pad(x, (72, 72), "constant", 0)  # パディングを追加
...
class DARUMA:
    def __init__(self,gpu_id):
        ...
        if gpu_id == "cpu":
            ...
            torch.cuda.set_device(int(gpu_id))  # 使用するGPUを指定
        ...
        self.model = CNN3_128_9_NN2_121_128(weight_parameters_path).to(self.device)  # モデル読み込み
        self.model.eval()  # 推論モード

        self.feature = load_AAindex()  # AAindexのロード
    ...
    def predict_from_sequence(self, seq, threshold=0.5, smoothing_window=17, remove_short_regions=True):
        ...
        with torch.no_grad(): # 勾配計算を無効化してメモリ消費を減らす
            prob = self.model(x).squeeze(0)[:, 1].tolist()  # 推論
